...
def main():
    ...
    choise_list = st.multiselect('언어를 선택하세요',lang_list)
    print(choise_list)

    if len(choise_list) != 0 :
        # 유저가 선택한 언어만, 차트를 그리려고 합니다.
        df_selected = df1[choise_list]
    ...
    # 스트림릿이 제공하는 바차트
    df_selectied2 = df2[['sepal_length','petal_length']]
    st.bar_chart(df_selectied2)

    # Altair 이용!
    # x 축과 y 축 설정 + color 또는 size로 차트를 풍성하게 표현
